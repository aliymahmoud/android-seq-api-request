...
public class Utilities {
    ...
    /**
     * Query the USGS dataset and return the json response.
     */
    ...
    public static ArrayList<earthQuake> extractEarthquakes(String jsonResponse) {
        ...
        try {
            ...
            JSONObject currentRecord,properties;
            int i;
            for(i = 0 ; i < features.length() ; i++)
            {
                currentRecord = features.getJSONObject(i);
                properties = currentRecord.getJSONObject("properties");

                earthquakes.add(new earthQuake((float) properties.getDouble("mag"),properties.getString("place"),
